;Credits MIKEOS http://http://mikeos.berlios.de, thanks guys!
;methanium methboot - bootloader for Ethanium kernel.
...
next_root_entry:
	...
	mov si, os_filename		; Start searching for OS filename
	mov cx, 11
	rep cmpsb
	...
	mov si, file_not_found		; If OS is not found, bail out
	call print_string
	jmp reboot
...
read_fat_ok:
	...
	mov ax, 2000h			; Segment where we'll load the OS
	mov es, ax
	mov bx, 0
...
next_cluster_cont:
	...
	mov dl, byte [bootdev]		; Provide OS with boot device info

	jmp 2000h:0000h			; Jump to entry point of loaded OS!
...
; STRINGS AND VARIABLES
	...
	pointer		dw 0 	; Pointer into Buffer, for loading OS
